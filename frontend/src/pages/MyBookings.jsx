    ...
    <div className="cosmic-bg min-h-[calc(100vh-64px)]">
      <div className="max-w-4xl mx-auto px-6 md:px-12 py-16 md:py-24">
        <div className="mb-10 fade-up">
          <p className="font-accent text-xs text-[#B8860B] mb-3">{t("nav.my_readings")}</p>
        ...
        {loading ? (
          <div className="text-center py-12"><Loader2 className="h-6 w-6 animate-spin text-[#B8860B] mx-auto" /></div>
        ) : bookings.length === 0 ? (
          <div className="glass-card p-10 text-center" data-testid="my-bookings-empty">
            <Calendar className="h-12 w-12 text-zinc-900 mx-auto mb-3" />
            <p className="font-body text-zinc-700">{t("scheduler.my_bookings_empty")}</p>
          </div>
        ) : (
            ...
            {bookings.map((b) => (
              <div key={b.id} className="glass-card p-5 flex items-center justify-between" data-testid={`my-booking-${b.id}`}>
                <div>
                  ...
                  <div className="font-body text-xs text-zinc-800 mt-1">
